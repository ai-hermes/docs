...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Register uses the same login endpoint (creates user if not exists)
    ...
    <Card className="w-full max-w-md mx-auto">
      ...
      <CardContent>
        ...
        <p className="mt-4 text-xs text-muted-foreground text-center">
          {t('terms')}{' '}
          <Link href={`/${locale}/terms`} className="text-purple-500 hover:underline">
            {t('termsLink')}
          </Link>{' '}
          {t('and')}{' '}
          <Link href={`/${locale}/privacy`} className="text-purple-500 hover:underline">
            {t('privacyLink')}
          </Link>
        </p>
